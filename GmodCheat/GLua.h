...
static const char* lua_typename[] =
{
	"nil",
	"bool",
	"lightuserdata",
...
{
private:
	virtual ILuaObject* CreateLuaObject(void) = 0;
	...
	virtual void LuaError(HLuaError *) = 0;
...
public:
	...
	virtual const char*	GetString(int iStackPos = -1, unsigned int* iOutLen = NULL) = 0; //23
...
public:
	...
	virtual void *Shutdown(void); // 1
	virtual void *DumpStats(void); // 2
	virtual void *CreateLuaInterface(unsigned char, bool); // 3
	virtual void *CloseLuaInterface(ILuaInterface *);
	virtual void padding00() = 0;
	virtual ILuaInterface *GetLuaInterface(unsigned char);
	virtual void *LoadFile(std::string  const&, std::string  const&, bool, bool);
	virtual void *GetCache(std::string  const&);
